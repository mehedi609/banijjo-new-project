            ...
            <div className="row">
              ...
              <div className="col-8">
                    ...
                    { colors.length > 0 && (
                      <div className="color-quality mt-3">
                       <h6>Color: {selectedColorName}</h6>
                        <div className="row">
                        {colors.map(({ colorId, imageName, colorName, seletected }) => (
                          <div 
                            key={colorId} 
                      ...
                      <div className="d-inline-block mr-5 mt-2">
                        ...
                        >
                          ...
                          {sizes.map(({ id, size, size_type_id }) => (
                            <option value={id} key={id}>{size}</option>
                          ))}
                        </select>
                      </div>
                    )}
                    ...
                    <div className="d-inline-block  mt-3">
                      ...
                      <div className="quantity">
                        <div className="quantity-select">
                          <div onClick={this.handleClickMinus} className="value-minus">&nbsp;</div>
                          <div className="value">
                            <span>{this.state.productQuantity}</span>
                          </div>
                          <div onClick={this.handleClickPlus} className="value-plus active">&nbsp;</div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="simpleCart_shelfItem mt-3">
                  ...
                  <button className="btn btn-outline-success rounded-0 mr-3" onClick={this.addToLocalStorage("cart")}>
                    Add to cart
                  </button>
                  <button className="btn btn-outline-success rounded-0" onClick={this.addToLocalStorage("wish")}>
                    Add to wish list
                  </button>
                </div>
...
export const getServerSideProps = async ({ params }) => {
  ...
  let combinations = await fetcher(`${base}/api/productCombinationsFromStock/${id}`)
  combinations = combinations.combinations

  let discountAmount = await fetcher(`${base}/api/getDiscountByProductId/${id}`)
  discountAmount = discountAmount.discountAmount

  let product_list_same_vendor_other_cat = await fetcher(`${base}/api/sameVendorOrCat/${id}/v`)
  product_list_same_vendor_other_cat = product_list_same_vendor_other_cat.sameVendorOrCat

  let product_list_same_category_other_ven = await fetcher(`${base}/api/sameVendorOrCat/${id}/c`)
  product_list_same_category_other_ven = product_list_same_category_other_ven.sameVendorOrCat
